...
def cdep(model, data, blobs,model_type = 'cifar'):
    rel, irrel = cdep.cd(blobs, data,model,model_type =model_type)
...
def gradient_sum(im, target, seg, model, crit, device='cuda'):
    ...
    Computes the sum of squared gradients for the specified inputs.
    '''
    # im.requires_grad = True
    # Compute gradients
    gradients = torch.autograd.grad(crit(model(im), target), im, create_graph=True)[0]
    # Sum along the color channels
    grad_sum = gradients.sum(dim=1)
    # Fix: Use torch.bool instead of .byte() for the mask
    mask = seg.bool()
    # Select gradients based on the segmentation mask and square them
    masked_grads = grad_sum.masked_select(mask)**2
    # Sum the squared gradients
    grad_params = masked_grads.sum()
    return grad_params
