...
for i in tqdm(range(num_samples)):
    if random.random()<=0.2:
        ...
        color_x[i ] = train_dataset.dataset.data[train_dataset.indices[i]].numpy().astype(np.float32)[np.newaxis]

    color_y[i] = train_dataset.dataset.targets[train_dataset.indices[i]]

os.makedirs("../../data/ColorMNIST", exist_ok = True)
np.save(oj("../../data/ColorMNIST", "clarc_setting_train_x.npy"), color_x)
np.save(oj("../../data/ColorMNIST", "clarc_setting_train_y.npy"), color_y)
...
for i in tqdm(range(num_samples)):
    ...
    if random.random()<=0.2:
        ...
        color_x[i ] = val_dataset.dataset.data[val_dataset.indices[i]].numpy().astype(np.float32)[np.newaxis]
    color_y[i] = val_dataset.dataset.targets[val_dataset.indices[i]]

os.makedirs("../../data/ColorMNIST", exist_ok = True)
np.save(oj("../../data/ColorMNIST", "clarc_setting_val_x.npy"), color_x)
np.save(oj("../../data/ColorMNIST", "clarc_setting_val_y.npy"), color_y)
